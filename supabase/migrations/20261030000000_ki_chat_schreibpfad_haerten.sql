...
comment on policy ki_chat_nachrichten_insert_own on public.ki_chat_nachrichten is
  'Eine angemeldete Person schreibt ausschliesslich ihre eigene Frage (rolle = nutzer, ohne Anbieternamen, ohne Fallback- oder Eskalationskennzeichen). Antworten und Eskalationen entstehen nur ueber ki_chat_antwort_schreiben()/ki_chat_eskalation_schreiben() (20261030000000).';
...
begin
  ...
  insert into public.ki_chat_nachrichten (profil_id, rolle, inhalt, anbieter_name, fallback)
  values (v_profil, 'assistent', p_inhalt, nullif(btrim(coalesce(p_anbieter_name, '')), ''), coalesce(p_fallback, false))
...
revoke all on function public.ki_chat_antwort_schreiben(text, text, boolean) from public;
revoke all on function public.ki_chat_eskalation_schreiben(text) from public;
grant execute on function public.ki_chat_antwort_schreiben(text, text, boolean) to authenticated;
