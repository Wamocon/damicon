...
2. **Hauptspalte:** Kopfzeile (`DashboardTopbar`) ab `md` mit Menü-Umschalter links, Sprachumschalter, Theme-Umschalter, Rollenumschalter „Ansicht als“ und „KI fragen“, dazu die Synchronisationsanzeige. Die angemeldete Person steht nicht mehr hier, sondern am Fuß der Seitenleiste. Darunter der Seiteninhalt.
   ...
   Die **globale Suche** (`components/suche/`) hat zwei Auslöser: ab `xl` einen Knopf im Look eines Suchfelds zwischen Pfad und Werkzeugen, darunter einen Lupenknopf direkt links neben der Glocke – auf jeder Breite, auch auf dem Handy. Getippt wird nie in der Kopfzeile, sondern im Fenster, das von oben aufgeht (`Sheet` `oben`). Es hängt neben der Shell im Layout und nicht in der Kopfzeile: deren `backdrop-blur` macht sie zum Bezugsrahmen für fixierte Kinder, ein Blatt darin wäre auf die Kopfzeile beschnitten. Stufe 1 findet Übersicht, Bereiche, Module und die Seiten daneben, gefiltert nach der Ansichtsrolle wie die Seitenleiste; Datensätze folgen (WMCNL-1467). Wird der Kopfzeile ab `xl` die Breite knapp, etwa mit angedocktem KI-Panel, misst sich der Feld-Knopf selbst (`@container`) und zeigt nur noch die Lupe.
