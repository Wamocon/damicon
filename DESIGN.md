...
| `Sheet` (`ui/sheet.tsx`) | Fläche, die von unten, in der Mitte oder von oben aufgeht | `unten` unter `md` für die untere Leiste, `mitte` für Detailinhalte (Tageskacheln), `oben` für das Suchfenster unter `xl`: mit `anker` genau an der Lupe in der Kopfzeile, ohne Anker (auf dem Handy) randlos oben. Trägt Esc, Klick daneben, Scroll-Sperre und eine Fokusfalle; `bg-card-deckend`, `max-h-[85svh]`. Optional `anfangsFokus` (etwa ein Suchfeld statt der Fläche), `kopf` (Inhalt statt des sichtbaren Titels, der Titel bleibt für Vorlesehilfen) und `schliessenLabel`. Die Scroll-Sperre sitzt am `<html>`, nicht am `<body>`: das `<html>` trägt `overflow-x: clip`, und ein gesperrter `<body>` wurde dann selbst zum Scrollcontainer, an dem die klebende Kopfzeile hing statt am Fenster. Hängt beim Schließen aus — was gemountet bleiben muss, wie das KI-Panel mit einer laufenden Antwort, baut seine Fläche selbst (`ki-pane.css`). |
...
2. **Hauptspalte:** Kopfzeile (`DashboardTopbar`) ab `md` mit Menü-Umschalter links, Sprachumschalter, Theme-Umschalter, Rollenumschalter „Ansicht als“ und „KI fragen“, dazu die Synchronisationsanzeige. Die angemeldete Person steht nicht mehr hier, sondern am Fuß der Seitenleiste. Darunter der Seiteninhalt.
   ...
   Die **globale Suche** (`components/suche/`) beginnt direkt hinter dem Pfad. Ab `xl` ist sie ein echtes Eingabefeld, das die Lücke bis zu den Werkzeugen füllt: man tippt direkt in der Kopfzeile, und die Treffer hängen als Liste darunter, so breit wie das Feld, mindestens 30rem, ohne Blende (`such-leiste.tsx`). Zwischen `md` und `xl` steht an derselben Stelle eine Lupe, auf dem Handy links neben der Glocke. Die Lupe öffnet ein Fenster dort, wo sie sitzt (`Sheet` `oben` mit `anker`): mit derselben Oberkante, notfalls nach links gerückt, damit es ganz im Bild bleibt, auf dem Handy oben über die volle Breite. Das Fenster hängt neben der Shell im Layout und nicht in der Kopfzeile: deren `backdrop-blur` macht sie zum Bezugsrahmen für fixierte Kinder, ein Blatt darin wäre auf die Kopfzeile beschnitten. Die Liste unter dem Feld ist nicht fixiert, sondern hängt absolut am Feld und darf deshalb in der Kopfzeile stehen. Feld und Fenster teilen sich die Suche (`such-sitzung.ts`) und die Trefferliste (`such-liste.tsx`). Stufe 1 findet Übersicht, Bereiche, Module und die Seiten daneben, gefiltert nach der Ansichtsrolle wie die Seitenleiste. Darunter stehen unter „Erwähnt in“ Module, deren Beschreibung oder Zusammenfassung den Begriff nennt. Durchsucht wird nur, was auf den Seiten zu lesen ist, und die Fundstelle steht als Beleg dabei. Datensätze folgen (WMCNL-1467). Wird der Kopfzeile ab `xl` die Breite knapp, etwa mit angedocktem KI-Panel, misst sich das Feld selbst (`@container`) und weicht unter 10rem der Lupe.
...
4. **Untere Navigationsleiste** (`untere-leiste.tsx`), nur unter `md`. Eine schwebende, abgerundete Fläche mit drei Knöpfen: Menü, KI-Assistent, Konto. Jeder führt eine eigene Fläche von unten herauf (`ui/sheet.tsx`), der KI-Knopf öffnet das bestehende Panel. Im Einzelnen:
   ...
   - **Stapelregel für die untere Bildschirmkante.** Wer dort etwas Festes ablegt, rechnet mit `--untere-leiste-raum` (`globals.css`). Die Variable ist ab `md` 0 und darunter Leistenhöhe plus Abstand plus `env(safe-area-inset-bottom)`; der Systemabstand ist der Home-Indicator des iPhone und die Gestenleiste von Android, ohne ihn wird ein Bedienelement zum Wischziel des Betriebssystems. Sie steht erst zur Verfügung, seit der Viewport-Export `viewportFit: "cover"` setzt. Heute lesen sie vier Stellen: die Leiste selbst, der untere Innenabstand der Hauptspalte, Himbi samt Versteck (`haustier.css`) und das Blatt von unten (`ui/sheet.tsx`, Unterkante und `max-h`).
     ...
     Das Suchfenster liegt ebenfalls auf `z-100`, aber anders als die Blätter der Leiste mit `aria-modal`: die Leiste steigt dafür nicht, sie liegt unter seiner Blende. Blätter und Suche schließen einander aus wie Blätter und KI-Panel – solange ein Blatt offen ist, öffnen die Tastenkürzel die Suche nicht. Die Trefferliste unter dem Feld ab `xl` braucht keine eigene Ebene: sie liegt im Stapel der Kopfzeile und damit über dem Inhalt.
...
- Symbolknöpfe der Kopfzeile unter `lg`: sichtbar 36 px wie ihre Nachbarn, fangen aber 44 px über ein Pseudoelement (`relative after:absolute after:-inset-1 lg:after:hidden`). Umgesetzt beim Suchknopf; bei 8 px Abstand berühren sich die Flächen zweier Nachbarn, ohne sich zu überlappen. Die Glocke hat es noch nicht.
- Tastenkürzel: `/` und Strg+K bzw. ⌘K führen in die globale Suche, wie im Handbuch: ab `xl` ins Feld der Kopfzeile, darunter ins Suchfenster. `/` nur außerhalb von Eingabefeldern. Strg+K erkennt auf russischer und kasachischer Belegung die Taste über `event.code`, weil `event.key` dort ein kyrillischer Buchstabe ist.
- Die Suche folgt dem WAI-ARIA-Muster Combobox mit Listbox: der Fokus bleibt im Feld, die Pfeiltasten verschieben nur die Markierung (`aria-activedescendant`), Enter öffnet. Im Fenster schließt Esc und gibt den Fokus an die Lupe zurück. Im Feld der Kopfzeile klappt Esc die Liste zu und lässt den Begriff stehen, ein zweites Esc leert das Feld; der Fokus bleibt darin. Die Trefferzahl liest eine `role="status"`-Region vor, sobald das Tippen ruht. Die markierte Zeile trägt zusätzlich zur Farbe einen Rahmen.
