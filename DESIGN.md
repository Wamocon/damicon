...
1. **Seitenleiste** links (`w-76`, 304 px): Bildmarke und Wortmarke, Kachel mit aktiver Rolle bzw. Persona, Link „Übersicht“, danach die Module in vier auf- und zuklappbaren Bereichsgruppen. Sichtbar ist nur, was die Rolle sehen darf (`hasPermission(role, resource, "view")`). Aktiver Eintrag: `bg-primary text-primary-foreground`. Mobil als Schublade (Menüknopf in der Kopfzeile). Im Einzelnen:
   ...
   - **Gruppe statt Einrückung.** Jede Gruppe ist eine eigene Fläche: Rahmen aus `color-mix(… 22 %, --sidebar-border)` und Grund aus `color-mix(… 5 %, transparent)` in der Bereichsfarbe, dazu die Symbolkachel bei 16 %. Eingerückt wird nicht – die Beschriftungen brauchen die Breite.
   ...
   - **„Übersicht“ ist ein fünftes Feld.** Sie steht auf derselben Ebene wie die vier Bereiche und bekommt deshalb dieselbe Fläche und dieselbe 24 × 24-Kachel, nur neutral statt in einer Bereichsfarbe (`--sidebar-border` als Rahmen, `--sidebar-accent/40` als Grund, Kachel in `--primary`) – sie gehört zu keinem Bereich. Ohne diese Fläche hing die Zeile sichtbar lose über vier Karten. Als aktive Seite füllt sie wie jeder aktive Eintrag `bg-primary`; die Kachel wechselt dann auf `bg-primary-foreground/20`, sonst verschwände sie im Grund.
