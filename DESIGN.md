...
2. **Hauptspalte:** Kopfzeile (`DashboardTopbar`) ab `md` mit Menü-Umschalter links, Sprachumschalter, Theme-Umschalter, Rollenumschalter „Ansicht als“ und „KI fragen“, dazu die Synchronisationsanzeige. Die angemeldete Person steht nicht mehr hier, sondern am Fuß der Seitenleiste. Darunter der Seiteninhalt.
   ...
   Die **globale Suche** (`components/suche/`) beginnt direkt hinter dem Pfad: ab `xl` als Knopf im Look eines Suchfelds, der die Lücke bis zu den Werkzeugen füllt, zwischen `md` und `xl` als Lupe an derselben Stelle. Auf dem Handy steht die Lupe links neben der Glocke. Getippt wird nie in der Kopfzeile, sondern im Fenster, und das geht dort auf, wo der Auslöser sitzt (`Sheet` `oben` mit `anker`): ab `xl` genau über dem Feld, die Treffer klappen darunter auf; an der Lupe mit derselben Oberkante, notfalls nach links gerückt, damit es ganz im Bild bleibt; auf dem Handy oben über die volle Breite. Das Fenster hängt neben der Shell im Layout und nicht in der Kopfzeile: deren `backdrop-blur` macht sie zum Bezugsrahmen für fixierte Kinder, ein Blatt darin wäre auf die Kopfzeile beschnitten. Stufe 1 findet Übersicht, Bereiche, Module und die Seiten daneben, gefiltert nach der Ansichtsrolle wie die Seitenleiste. Darunter stehen unter „Erwähnt in“ Module, deren Beschreibung oder Zusammenfassung den Begriff nennt. Durchsucht wird nur, was auf den Seiten zu lesen ist, und die Fundstelle steht als Beleg dabei. Datensätze folgen (WMCNL-1467). Wird der Kopfzeile ab `xl` die Breite knapp, etwa mit angedocktem KI-Panel, misst sich der Feld-Knopf selbst (`@container`) und zeigt nur noch die Lupe.
...
4. **Untere Navigationsleiste** (`untere-leiste.tsx`), nur unter `md`. Eine schwebende, abgerundete Fläche mit drei Knöpfen: Menü, KI-Assistent, Konto. Jeder führt eine eigene Fläche von unten herauf (`ui/sheet.tsx`), der KI-Knopf öffnet das bestehende Panel. Im Einzelnen:
   ...
   - **Stapelregel für die untere Bildschirmkante.** Wer dort etwas Festes ablegt, rechnet mit `--untere-leiste-raum` (`globals.css`). Die Variable ist ab `md` 0 und darunter Leistenhöhe plus Abstand plus `env(safe-area-inset-bottom)`; der Systemabstand ist der Home-Indicator des iPhone und die Gestenleiste von Android, ohne ihn wird ein Bedienelement zum Wischziel des Betriebssystems. Sie steht erst zur Verfügung, seit der Viewport-Export `viewportFit: "cover"` setzt. Heute lesen sie vier Stellen: die Leiste selbst, der untere Innenabstand der Hauptspalte, Himbi samt Versteck (`haustier.css`) und das Blatt von unten (`ui/sheet.tsx`, Unterkante und `max-h`).
     ...
     Das Suchfenster liegt ebenfalls auf `z-100`, aber anders als die Blätter der Leiste mit `aria-modal`: die Leiste steigt dafür nicht, sie liegt unter seiner Blende. Blätter und Suche schließen einander aus wie Blätter und KI-Panel – solange ein Blatt offen ist, öffnen die Tastenkürzel die Suche nicht.
...
- Tastenkürzel: `/` und Strg+K bzw. ⌘K öffnen die globale Suche, wie im Handbuch. `/` nur außerhalb von Eingabefeldern. Strg+K erkennt auf russischer und kasachischer Belegung die Taste über `event.code`, weil `event.key` dort ein kyrillischer Buchstabe ist.
